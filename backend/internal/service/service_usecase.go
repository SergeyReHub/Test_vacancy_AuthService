...
import (
	...
	suppfunc "auth/backend/internal/service/supp_func"
...
func (a *AuthServiceImpl) TakeBothTokens(guid string, userAgent string, ctx context.Context) (*models.Tokens, error) {
	...
	err = a.Repo.InsertRefreshToken(refreshToken, claims, userAgent, ctx)
	if err != nil {
...
func (a *AuthServiceImpl) RefreshTokens(refresh_token string, access_token string, userAgent string, ctx context.Context) (*models.Tokens, error) {
	...
	if err != nil {
		...
		a.Logger.Info("Error check user agent", zap.Error(err))
		return nil, err
	}
	if !b {
		a.Logger.Info("changed user-agent", zap.String("userAgent", userAgent)) //asdasd
	...
	newAccessToken, claims, err := suppfunc.GenerateJwtToken(guid, newRefreshToken, a.Config)
	if err != nil {
		a.Logger.Error("Error generate JWT access token.", zap.Error(err))
		return nil, err
	}
	...
	err = a.Repo.InsertRefreshToken(refresh_token, claims, userAgent, ctx)
	if err != nil {
		a.Logger.Error("Error insert refresh token.")
		return nil, err
	}
...
func (a *AuthServiceImpl) Deauthorization(accessToken string, ctx context.Context) error {
	token, err := jwt.ParseWithClaims(accessToken, &models.ClaimsJWT{}, func(token *jwt.Token) (interface{}, error) {
	...
	claims, ok := token.Claims.(*models.ClaimsJWT)
	if ok {
