...
func GenerateJwtToken(guid string, refresh_token string, cfg *config.Config) (string, *models.ClaimsJWT, error) {
...
func ValidateJWT(tokenString string, refreshToken string, cfg *config.Config) (string, error) {
	token, err := jwt.ParseWithClaims(tokenString, &models.ClaimsJWT{}, func(token *jwt.Token) (interface{}, error) {
	...
	if claims, ok := token.Claims.(*models.ClaimsJWT); ok && claims.RefreshToken == refreshToken {
