...
import (
	...
	"auth/backend/internal/repository/postgres_storage"
...
func (repo *Repository) InsertRefreshToken(refreshToken string, claims *models.ClaimsJWT, userAgent string, ctx context.Context) error {
	issued_at := claims.IssuedAt.Time
	expires_at := claims.ExpiresAt.Time
	user_guid := claims.GUID

	err := repo.PostgresStorage.InsertRefreshToken(refreshToken, expires_at, issued_at, user_guid, userAgent, ctx)
	if err != nil {
		return err
	}
	return nil
...
func (repo *Repository) CreateUser(user *models.User, ctx context.Context) (error){
